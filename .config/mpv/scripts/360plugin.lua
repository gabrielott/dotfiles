...
local closeCurrentLog = function()
	...
	if lasttimePos ~= nil and file_object ~= nil then
		...
		local closingCommandComment = string.format(
			'ffmpeg -hide_banner -y -ss %s -i "%s" -to %s -copyts -filter_complex "%sv360=%s:%s:in_stereo=%s:out_stereo=%s:reset_rot=1:id_fov=%s:d_fov=%.3f:yaw=%.3f:pitch=%.3f:roll=%.3f:w=1920.0:h=1080.0:interp=cubic:h_flip=%s,setsar=sar=%.3f,sendcmd=filename=%s_3dViewHistory_%s.txt" -avoid_negative_ts make_zero -preset slower -crf 17 "%s_2d_%03d.mp4"',
...
local onExit = function()
	...
	if mergedCommand ~= '' then
		...
		print(mergedCommand)
		batchfile = io.open('convert_3dViewHistory.sh', 'w')
		batchfile:write("#!/bin/sh\n\n")
		batchfile:write("parallel -j2 --bar <<- EOF\n")
